...
Keyboard shortcuts: `Backspace` goes up, `Alt+Left` and `Alt+Right` move through history, `F5` rescans. The mouse wheel zooms the chart towards the cursor; dragging with the middle mouse button pans it, and a middle-button double click restores the initial view.
