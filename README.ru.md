...
- Настройки сохраняются между запусками: метрика диаграммы, цветовая схема, флажок **Follow in tree**, размер и положение окна, ширина дерева, а также последний просканированный путь, который подставляется в поле пути (и выбирается в списке дисков, если это диск) без запуска сканирования. Настройки хранятся в `%APPDATA%\Disk Flashlight\data\app.ron`.
