...
- Параллельный обход каталогов для подкаталогов, томов без NTFS и запуска без прав администратора (тот же диск `C:` сканируется около 8 секунд на холодном кэше и около 1,3 секунды на прогретом). Если целый NTFS-диск сканируется без прав администратора, кнопка **Fast scan** на панели инструментов (отмечена щитом UAC) перезапускает приложение с повышенными правами и повторно сканирует диск через MFT.
