...
GitHub releases: `.github/workflows/release.yml` runs `make_release.py` on `windows-latest` for tags `v*` (tag must equal `v` + Cargo.toml version, otherwise the job fails) and publishes `dist/*_Setup.exe` plus `dist/*_portable.zip` (only the exe inside a `Disk Flashlight` folder, built by `make_release.py::make_portable_zip`); `workflow_dispatch` builds an artifact only. Creating or pushing a tag publishes a public release, so do it only when the user asks.
...
- **`scan/walk.rs`** walks directories under rayon, listing each with `win::list_dir`: `GetFileInformationByHandleEx(FileFullDirectoryInfo)` into a 64 KiB thread-local buffer (names, attributes, sizes, reparse tag in `EaSize`), about 40% faster than `std::fs::read_dir` on a warm cache and verified entry-for-entry against it. Symlinks and junctions (name-surrogate reparse points, the same rule as `FileType::is_symlink`) are skipped. The listing's `AllocationSize` is deliberately not used: it is stale for secondary hard links and rounded to 64 KiB for sparse files, so physical size stays cluster-rounded, with `GetCompressedFileSizeW` for compressed or sparse files. It builds a nested `RawDir` tree and reports progress through atomics in `Progress`. `scan/mod.rs` runs it on a thread and hands the result back over a bounded channel; `scan/win.rs` wraps the few Win32 calls (drive list, cluster size, compressed size). `windows-sys` features are kept minimal (`Win32_Foundation`, `Win32_Storage_FileSystem`, `Win32_System_IO`, `Win32_UI_Shell`); `DRIVE_*` constants are hard-coded to avoid another feature. `win::relaunch_elevated` restarts the app via `ShellExecuteW("runas")`; arguments go through `app::quote_arg` because `CommandLineToArgvW` treats `\"` as an escaped quote.
